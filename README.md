...
### "Token is missing required scope" / `missing scope: operator.read`

This error means your OpenClaw token exists but doesn't have the operator-level permissions that PinchChat requires. PinchChat always requests the following scopes: `operator.read`, `operator.write`, `operator.admin`, `operator.approvals`.

The default token generated by OpenClaw's initial setup may not include these scopes. To fix it:

1. Open your `openclaw.json` configuration file on the server
2. Find the token entry you're using to log in
3. Make sure it includes at least `operator.read` and `operator.write` in its `scopes` array
4. Restart OpenClaw after saving the config

Example token configuration in `openclaw.json`:
```json
{
  "tokens": [
    {
...
Refer to the [OpenClaw documentation](https://github.com/openclaw/openclaw) for the exact syntax used by your version.
